...
@implementation HippyBaseListView3Cell
...
@interface HippyBaseListView3() <HippyScrollProtocol, HippyRefreshDelegate>


@end

@implementation HippyBaseListView3 {
...
- (instancetype)initWithBridge:(HippyBridge *)bridge
{
    if (self = [super initWithFrame: CGRectZero])
    {
        ...
        _dataSource = [HippyBaseListView3DataSource new];
...
{
    return [HippyBaseListView3Cell class];
}
...
- (void)initTableView
{
    if (_tableView3 == nil) {
        ...
        _tableView3.autoresizingMask = UIViewAutoresizingFlexibleWidth | UIViewAutoresizingFlexibleHeight;
        _tableView3.backgroundColor = [UIColor clearColor];
        _tableView3.allowsSelection = NO;
...
- (void)reloadData
{
    ...
    if (self.initialContentOffset) {
        [_tableView3 setContentOffset:CGPointMake(0, self.initialContentOffset) animated:NO];
...
- (void)insertHippySubview:(UIView *)subview atIndex:(NSInteger)atIndex
{
    if ([subview isKindOfClass:[HippyHeaderRefresh class]]) {
        ...
        [_headerRefreshView setScrollView:self.tableView3];
        ...
        [_footerRefreshView setScrollView:self.tableView3];
...
{
    [_tableView3 setScrollEnabled:value];
}
...
- (CGSize)contentSize
{
    return self.tableView3.contentSize;
}
...
{
    [self.tableView3 setContentOffset: offset animated: animated];
}
...
- (void)scrollToIndex:(NSInteger)index animated:(BOOL)animated
{
    ...
    if (indexPath != nil) {
        [self.tableView3 scrollToRowAtIndexPath: indexPath atScrollPosition: UITableViewScrollPositionTop animated: animated];
    }
}
...
- (CGFloat)tableView3:(__unused UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath;
{
    HippyVirtualCell *cell = [_dataSource cellForIndexPath: indexPath];
    return ceil(CGRectGetHeight(cell.frame));
}

- (NSInteger)tableView3:(__unused UITableView *)tableView numberOfRowsInSection:(NSInteger)section
{
    return [_dataSource numberOfCellForSection: section];
}

- (void)tableView3:(UITableView *)tableView willDisplayCell:(UITableViewCell *)cell forRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([cell isKindOfClass:[HippyBaseListView3Cell class]]) {
        [[(HippyBaseListView3Cell *)cell cellView] viewAppearEvent];
    }
}

- (void)tableView3:(UITableView *)tableView didEndDisplayingCell:(UITableViewCell *)cell forRowAtIndexPath:(NSIndexPath*)indexPath {
    if ([cell isKindOfClass:[HippyBaseListView3Cell class]]) {
        [[(HippyBaseListView3Cell *)cell cellView] viewDisappearEvent];
    }
}
...
- (UITableViewCell *)tableView3:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    HippyBaseListView3Cell *cell = (HippyBaseListView3Cell *)[tableView dequeueReusableCellWithIdentifier: identifier];
    while (cell && !([[(HippyVirtualCell *)cell.node itemViewType] isEqualToString: newNode.itemViewType])) {
        // 此处cell还在tableView上，将导致泄漏
        [cell removeFromSuperview];
        cell =(HippyBaseListView3Cell *)[tableView dequeueReusableCellWithIdentifier: identifier];
        if (cell == nil) {
...
{
    return @{@"contentOffset": @{@"x": @(_tableView3.contentOffset.x), @"y": @(_tableView3.contentOffset.y)}};
}
...
- (void)setBounces:(BOOL)bounces {
    [_tableView3 setBounces:bounces];
}

- (BOOL)bounces {
    return [_tableView3 bounces];
}

- (void)setShowScrollIndicator:(BOOL)show {
    [_tableView3 setShowsVerticalScrollIndicator:show];
}

- (BOOL)showScrollIndicator {
    return [_tableView3 showsVerticalScrollIndicator];
}
